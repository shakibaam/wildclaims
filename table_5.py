...
def main():
    ...
    # # Collect all values across methods
    # all_gold_values = []
    # all_major_24_values = []
    # all_hassan_15_values = []
    # all_intersection_values = []
    # all_union_values = []
    
    # for data in claim_methods.values():
    #     all_gold_values.extend(data['gold_values'])
    #     all_major_24_values.extend(data['major_24_binary_values'])
    #     all_hassan_15_values.extend(data['hassan_15_binary_values'])
    #     all_intersection_values.extend(data['intersection_values'])
    #     all_union_values.extend(data['union_values'])
    
    # # Define the columns to analyze for overall statistics
    # overall_columns_to_analyze = [
    #     ('major_24_binary', all_major_24_values),
    #     ('hassan_15_binary', all_hassan_15_values),
    #     ('Intersection', all_intersection_values),
    #     ('Union', all_union_values)
    # ]
    
    # for column_name, column_values in overall_columns_to_analyze:
    #     print(f"--- Overall {column_name} vs Gold ---")
        
    #     # Calculate confusion matrix
    #     true_positives = sum(1 for pred, gold in zip(column_values, all_gold_values) if pred and gold)
    #     false_positives = sum(1 for pred, gold in zip(column_values, all_gold_values) if pred and not gold)
    #     false_negatives = sum(1 for pred, gold in zip(column_values, all_gold_values) if not pred and gold)
    #     true_negatives = sum(1 for pred, gold in zip(column_values, all_gold_values) if not pred and not gold)
        
    #     # Calculate precision, recall, F1
    #     precision, recall, f1 = calculate_precision_recall_f1(true_positives, false_positives, false_negatives)
        
    #     # Calculate Kappa
    #     observed_agreement = (true_positives + true_negatives) / total_rows
        
    #     # Expected agreement
    #     pred_true_rate = sum(column_values) / len(column_values) if len(column_values) > 0 else 0
    #     pred_false_rate = 1 - pred_true_rate
    #     gold_true_rate = sum(all_gold_values) / len(all_gold_values) if len(all_gold_values) > 0 else 0
    #     gold_false_rate = 1 - gold_true_rate
