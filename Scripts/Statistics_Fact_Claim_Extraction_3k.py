...
def main():
    ...
    FHou_hassan_arrays = []
    FSong_hassan_arrays = []
    conversation_data = defaultdict(lambda: {'FHou_hassan': [], 'FSong_hassan': []})
    total_rows = 0
    ...
    with open(csv_file_path, 'r', encoding='utf-8') as file:
        ...
        for row in reader:
            ...
            FHou_hassan_str = row['FHou_hassan']
            FSong_hassan_str = row['FSong_hassan']
            
            # Parse arrays
            FHou_hassan_array = parse_array_string(FHou_hassan_str)
            FSong_hassan_array = parse_array_string(FSong_hassan_str)
            
            # Store arrays
            FHou_hassan_arrays.append(FHou_hassan_array)
            FSong_hassan_arrays.append(FSong_hassan_array)
            
            # Store by conversation
            conversation_data[conversation_hash]['FHou_hassan'].append(FHou_hassan_array)
    ...
    total_FHou_hassan_elements = sum(len(arr) for arr in FHou_hassan_arrays)
    print(f"1. Total number of elements in FHou_hassan arrays: {total_FHou_hassan_elements}")
    ...
    avg_FHou_hassan_elements = total_FHou_hassan_elements / total_rows if total_rows > 0 else 0
    print(f"3. Average number of elements in FHou_hassan arrays: {avg_FHou_hassan_elements:.2f}")
    ...
    FHou_elements_per_conversation = []
    for conv_hash, data in conversation_data.items():
        total_elements = sum(len(arr) for arr in data['FHou_hassan'])
        FHou_elements_per_conversation.append(total_elements)
    
    avg_FHou_elements_per_conversation = sum(FHou_elements_per_conversation) / len(FHou_elements_per_conversation) if FHou_elements_per_conversation else 0
    print(f"5. Average number of elements in FHou_hassan arrays per conversation: {avg_FHou_elements_per_conversation:.2f}")
    ...
    # 7. Percentage of rows with non-empty FHou_hassan arrays
    non_empty_FHou_hassan = sum(1 for arr in FHou_hassan_arrays if len(arr) > 0)
    pct_non_empty_FHou_hassan = (non_empty_FHou_hassan / total_rows) * 100 if total_rows > 0 else 0
    print(f"7. Percentage of rows with non-empty FHou_hassan arrays: {pct_non_empty_FHou_hassan:.2f}%")
    print(f"   - Non-empty rows: {non_empty_FHou_hassan} out of {total_rows}")
    ...
    # 9. Group by conversation_hash: Percentage of conversations with non-empty FHou_hassan
    conversations_with_FHou_hassan = 0
    total_conversations = len(conversation_data)
    
    for conv_hash, data in conversation_data.items():
        # Check if any turn in this conversation has non-empty FHou_hassan array
        has_non_empty_FHou = any(len(arr) > 0 for arr in data['FHou_hassan'])
        if has_non_empty_FHou:
            conversations_with_FHou_hassan += 1
    
    pct_conversations_FHou_hassan = (conversations_with_FHou_hassan / total_conversations) * 100 if total_conversations > 0 else 0
    print(f"9. Percentage of conversations with non-empty FHou_hassan arrays: {pct_conversations_FHou_hassan:.2f}%")
    print(f"   - Conversations with non-empty FHou_hassan: {conversations_with_FHou_hassan} out of {total_conversations}")
